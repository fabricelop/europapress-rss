...
function queueCard(x,processing){const a=document.createElement('article');a.className='queuecard';const details=x.source_details||[];const chips=(details.length?details:(x.sources||[]).map(s=>({source:typeof s==='string'?s:(s.name||s.source||'')}))).map(s=>s.url?'<a class="chip" target="_blank" rel="noopener" href="'+esc(s.url)+'" title="'+esc(s.title||s.source||'')+'">'+esc(s.source||'Fuente')+'</a>':'<span class="chip">'+esc(s.source||'Fuente')+'</span>').join('');a.innerHTML='<h3>'+esc(x.title||'Sin titular')+'</h3><p>'+esc(processing?'En elaboración':'Pendiente de una cuarta fuente')+' · '+esc(processing?((x.source_count||0)+' fuentes · '+when(x.selected_at)):('última detección '+when(x.last_seen)))+'</p><div class="chips">'+chips+'</div>'+(x.url?'<p><a class="source-link" target="_blank" rel="noopener" href="'+esc(x.url)+'">Abrir noticia base</a></p>':'');return a}
