...
(async () => {
  ...
  const result={query,search_url:url,fetched_at:new Date().toISOString(),authenticated_cookie_pair_present:Boolean(authToken&&ct0),mode:backfillDays>0?`backfill_${backfillDays}d`:'incremental',final_url:null,title:null,extracted:0,scroll_rounds:0,already_seen:0,rejected:[],candidates:[],status:'unknown',note:null};
  ...
  const rejectReason=text=>{
    ...
    if (/\brecordadme\s+(con\s+)?(carino|amor|afecto|ternura)\b/.test(t)) return '“recordadme con cariño/afecto…” se refiere a recordar a la persona';
  ...
  try{
    await page.goto(url,{waitUntil:'domcontentloaded',timeout:60000});await page.waitForTimeout(7000);result.final_url=page.url();result.title=await page.title();const loginVisible=await page.locator('text=Inicia sesión').first().isVisible().catch(()=>false);const extractedMap=new Map();let stableRounds=0,seenBoundaryRounds=0;const maxScrollRounds=backfillDays>0?80:12;
    for(let round=0;round<maxScrollRounds;round++){const visible=await readVisibleTweets();const before=extractedMap.size;for(const tweet of visible)extractedMap.set(tweet.id,tweet);const added=extractedMap.size-before;result.scroll_rounds=round+1;if(added===0)stableRounds++;else stableRounds=0;if(backfillDays>0){const dated=visible.map(t=>t.datetime?Date.parse(t.datetime):NaN).filter(Number.isFinite);if(dated.length&&Math.min(...dated)<=backfillCutoff)break;if(stableRounds>=4)break;}else{const visibleIds=visible.map(t=>t.id);const reachedSeenBoundary=visibleIds.some(id=>Boolean(seen[id]));if(reachedSeenBoundary)seenBoundaryRounds++;else seenBoundaryRounds=0;if(seenBoundaryRounds>=2)break;if(stableRounds>=3)break;}await page.evaluate(()=>window.scrollTo(0,document.body.scrollHeight));await page.waitForTimeout(1400);}
    let extracted=Array.from(extractedMap.values());if(backfillDays>0)extracted=extracted.filter(t=>!t.datetime||Date.parse(t.datetime)>=backfillCutoff);result.extracted=extracted.length;
